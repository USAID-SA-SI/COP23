...
# MUNGE -------------------------------------------------------------------
  ...
  # Overall TX_CURR targets over time
    ...
    df_cscd <-  df_cscd %>% 
      mutate(short_name = str_remove_all(short_name, " DM| MM")) %>% 
      mutate(short_name = case_when(short_name == "gp City of Johannesburg" ~ "gp COJ", 
                                    short_name == "wc City of Cape Town" ~ "wc CPT", 
                                    short_name == "fs Thabo Mofutsanyana" ~ "fs Thabo M", 
                                    short_name == "nw Ngaka Modiri Molema" ~ "nw Ngaka", 
                                    short_name == "nw Dr Kenneth Kaunda" ~ "nw DKK", 
                                    short_name == "gp City of Tshwane" ~ "gp Tshwane",
                                    short_name == "nw Bojanala Platinum" ~ "nw Bojanala", 
                                    short_name == "kz King Cetshwayo" ~ "kz KCD",
                                    TRUE ~ short_name)) %>% 
   ...
   df_tx_agesex_viz <- df_tx %>% 
     janitor::clean_names() %>% 
     rename(`FY23 TX_CURR Result excl PS` = tx_curr_2023_cy2023q3,
     ...
     pivot_longer(cols = c(`FY23 TX_CURR Result excl PS`:`FY26 TX_CURR Target`), names_to = "indicator") %>% 
...
df_tx_full_viz <-  df_tx_full %>% 
  ...
  rename(TX_CURR = cop24t_tx_curr_ps,
         TX_NEW = tx_new_ps,
         TX_PVLS_D = tx_pvls_d_ps,
         TX_PVLS_N = tx_pvls_n_ps,
         HTS_TST_POS = hts_pos_ps) %>% 
  summarise_if(is.double, sum, na.rm = TRUE) %>% 
  pivot_longer(cols = TX_CURR:TX_PVLS_N,
...
#metro_name <- c("gp City of Johannesburg MM", "kz eThekwini MM", "gp Ekurhuleni MM")
 ...
 df_tx_psnu_viz %>% 
   ...
   df_tx_agesex_viz %>%
     filter(indicator == "FY23 TX_CURR Result excl PS" | 
  ...
  # ART Coverage lollipop
   ...
   df_psnu_cov %>% 
     ...
     scale_fill_identity() +
     scale_color_identity() +
     scale_y_continuous(label = percent, limits = c(0,1.2)) +
     labs(x = NULL, y = NULL,
  ...
  si_save("Graphics/03a_art_cov_psnu_excl_ps.svg")
   ...
   df_agesex_cov %>% 
     ...
     coord_flip() + 
     si_style_nolines() +
     scale_y_continuous(label = label_number(scale_cut = cut_short_scale()), limits = c(0,1.1)) + 
     geom_text(aes(y = value,
                   label = percent(value, 1),
                   color = fill_color),
               family = "Source Sans Pro",
   ...
   df_agesex_cov %>% 
     ...
     scale_color_identity() +
     # scale_color_identity() +
     scale_y_continuous(label = percent, limits = c(0,1.1)) +
     labs(x = NULL, y = NULL,
  ...
  # Table for growth rates + abs #
   ...
   df_growth_agesex %>% 
     ...
     mutate(vjust_df = ifelse(pop == "<15", -2, 2),
            peds_lab_adj = ifelse(pop == "<15" & fy == "COP22", -2, -0.5),
            text_color = ifelse(pop == "<15", trolley_grey, "white")) %>% 
...
#   pull(short_name)
   ...
   df_growth_psnu %>% 
     ...
     # coord_flip() + 
     si_style_nolines() +
     scale_y_continuous(label = label_number(scale_cut = cut_short_scale()), limits = c(-30000, 80000)) + 
     geom_text(aes(y = abs_diff_incl_ps,
                   label = clean_number(abs_diff_incl_ps, 1),
                   #vjust = vjust_lab_adj,
                   color = fill_color),
...
df_tx_full_viz %>% 
  ...
  mutate(fill_color = case_when(indicator=="TX_CURR" ~ genoa, 
                                indicator=="TX_NEW" ~ genoa_light, 
                                indicator=="TX_PVLS_D" ~ scooter,
                                indicator=="TX_PVLS_N" ~ scooter_med, 
                                indicator=="HTS_TST_POS" ~ scooter_light)) %>% 
  ggplot(aes(x = fct_relevel(indicator, (order)), y = value, fill = fill_color)) + 
  geom_col() + 
  scale_y_continuous(label = label_number(scale_cut = cut_short_scale())) + 
  geom_text(aes(y = value,
